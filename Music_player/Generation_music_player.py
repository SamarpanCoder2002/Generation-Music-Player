...
class MusicPlayer:
    def __init__(self, window, backward_btn_img, play_btn_img, pause_btn_img, stop_image_btn, forward_btn_img):
        self.window = window
        ...
        self.forward_btn = forward_btn_img
        self.backward_btn = backward_btn_img
        self.pause_btn = pause_btn_img
        self.play_btn = play_btn_img
    ...
    def make_listbox(self):
        ...
        self.my_list_song = Listbox(self.window,bg="brown",fg="gold",width=120,height=10,
                                    font=("Arial",8,"bold"),relief=SUNKEN,borderwidth=20)
        self.my_list_song.place(x=25,y=80)

    def all_labels(self):
        self.heading_label = Label(self.window,text="Generation Music Player",
                                   font=("Arial",20,"bold","italic"),bg="orange",fg="blue")
        self.heading_label.place(x=240,y=25)

        self.empty_indicator = Button(self.window,text="Empty List",relief=FLAT,bd=10,command=self.add_many_song,
                                   font=("Arial",43,"bold","italic"),bg="brown",fg="gold")
        self.empty_indicator.grid(row=0,column=0,padx=220,pady=105,ipadx=10,ipady=10)

        self.empty_indicator.bind('<Enter>', self.On_hover_effect)
        self.empty_indicator.bind('<Leave>', self.Out_hover_effect)

    def text_button_set(self):
        self.add_one_song = Button(self.window,text="Add Song",font=("Helvetica",12,"bold","italic"),
                                   bg="blue",fg="red",width=12,relief=RAISED,borderwidth=13,command=self.add_many_song)
        self.add_one_song.place(x=480,y=352)

        self.add_multiple_song = Button(self.window, text="Delete Selected Song", font=("Helvetica", 12, "bold", "italic"),
                                   bg="blue", fg="red", width= 28, relief=RAISED, borderwidth=13,command=self.delete_selected_song)
        self.add_multiple_song.place(x=478, y=432)

        self.total_song = Button(self.window, text="Song Counter", width="11",font=("Helvetica", 12, "bold", "italic"),
                                    bg="blue", fg="red", relief=RAISED, borderwidth=13, command=self.song_counter)
    ...
    def On_hover_effect(self,e):
        self.empty_indicator.config(bg="green",fg="orange",relief=RAISED,bd=10)


    def Out_hover_effect(self,e):
    ...
    def song_duration_time(self):
        ...
        if self.song_length == converted_time  and self.repeat_counter == 1:
           ...
           status_bar.after(1000,self.song_duration_time)
    ...
    def muter(self):
        self.mute_scale = Scale(self.window,from_=1,to=0,orient=HORIZONTAL,bg="tan",command=self.get_mute,
                activebackground="red",font=("Arial",15,"bold"),length=47,relief=RIDGE,bd=3)
    ...
    def repeat_controller(self):
        self.loop_bar = Scale(self.window,from_=1,to=0,orient=HORIZONTAL,bg="tan",command=self.repeat_maintain,
                activebackground="red",font=("Arial",15,"bold"),length=140,relief=RIDGE,bd=3)
    ...
    def shuffle_controller(self):
        self.shuffle_bar = Scale(self.window,from_=1,to=0,orient=HORIZONTAL,bg="tan",command=self.shuffle_maintain,
                activebackground="red",font=("Arial",15,"bold"),length=140,relief=RIDGE,bd=3)
    ...
    def create_menu(self):
        ...
        color_menu.add_checkbutton(variable=self.fg_custom_color,label="Foreground Color change", activebackground="black", activeforeground="red",
                                   font=("Arial", 10, "bold"), foreground="blue",command=self.fg_color_change)
        color_menu.add_checkbutton(variable=self.default_bg_color,label="Default Background Color", activebackground="black", activeforeground="red",
                                   font=("Arial", 10, "bold"), foreground="blue",command=self.default_background_color)
        color_menu.add_checkbutton(variable=self.default_fg_color,label="Default Foreground Color", activebackground="black",
                                   activeforeground="red", font=("Arial", 10, "bold"), foreground="blue", command=self.default_foreground_color)

        option_menu = Menu(self.music_menu)
    ...
    def change_heading(self):
        ...
        new_label_heading = Label(self.heading_window,text="Enter the new heading",bg="tan",fg="brown",
                                  font=("Helvetica",25,"bold","italic"))
        new_label_heading.place(x=25,y=100)

        self.new_label_entry = Entry(self.heading_window,font=("Arial",17,"bold"),fg="red",bg="black",width=25,
                                relief=SUNKEN,bd=15)
    ...
    def active_target_song(self):
        ...
        new_label_heading = Label(self.heading_window, text="-:Enter the target song number:-", bg="tan", fg="brown",
                                  font=("Helvetica", 18, "bold", "italic"))
        new_label_heading.place(x=15, y=100)

        self.new_song_entry = Entry(self.heading_window, font=("Arial", 17, "bold"), fg="red", bg="black", width=25,
                                     relief=SUNKEN, bd=15)
        self.new_song_entry.focus_force()
        self.new_song_entry.place(x=25, y=170)

        ok_btn = Button(self.heading_window, text="Ok", bg="green", fg="yellow", relief=RAISED, bd=10,
                        font=("Arial", 13, "bold"), padx=10, pady=7, command=lambda: self.target_song_set('<Return>'))
        ok_btn.place(x=170, y=260)
